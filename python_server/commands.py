...
def execute_command(data,addr,sock):
    try:
        ...
        # ── Chat Messages from Phone ──
            ...
            chat_payload = command.split(":", 1)[1]
            # New format: CHAT_MSG:msg_id:text  (backward compat: old format has no msg_id prefix)
            if chat_payload.startswith("msg_"):
                parts = chat_payload.split(":", 1)
                msg_id = parts[0]
        ...
        # ── Chat Sync Request from Phone ──
        elif command.startswith("CHAT_SYNC:"):
            try:
                since_str = command.split(":", 1)[1].strip()
                since_ts = int(since_str) if since_str and since_str != "0" else 0
                from chat_module import ChatStorage
                storage = ChatStorage()
                missed = storage.get_messages_since(since_ts)
                if missed:
                    import json
                    response = "CHAT_SYNC:" + json.dumps(missed, ensure_ascii=False)
                    reverse_commands.send_to_phone(response)
                    print(f"[*] Sent {len(missed)} missed chat message(s) to phone")
                else:
                    print("[*] Chat sync: phone is up to date")
            except Exception as e:
        ...
        # ─── SYNC HANDSHAKE ─────────────────────────────────────────
        elif command.startswith("SYNC_HANDSHAKE:"):
            # Incremental sync handshake from phone
            try:
                import sync_manager
                handshake_payload = command.split(":", 1)[1]
                delta = sync_manager.handle_handshake(handshake_payload)
                response = sync_manager.build_sync_delta_response(delta)
                # Send the delta back to the phone
                reverse_commands.send_to_phone(response)
                print(f"[*] Sent sync delta to phone: {len(delta.get('tasks', []))} task(s)")
            except Exception as e:
                print(f"[!] Sync handshake error: {e}")
        ...
        # ─── AUDIT LOG COMMAND (Feature 16) ──────────────────────────
        elif command.startswith("GET_AUDIT_LOG:"):
            try:
                import task_audit
                task_id = command[len("GET_AUDIT_LOG:"):].strip()
                entries = task_audit.get_log_for_task(task_id, limit=50)
