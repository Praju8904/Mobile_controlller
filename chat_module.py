...
# ─── GLOBAL REFERENCE ───────────────────────────────────────────
# This allows other threads (like services.py) to find the active window
_active_tab = None 
...
class ChatTab:
    ...
    def _render_file(self, bubble, content, metadata, is_me):
        ...
        # Open button (Bright and visible)
        if os.path.exists(filepath):
            ctk.CTkButton(
                inner, text="OPEN FILE", width=80, height=26, 
                font=("Segoe UI", 10, "bold"),
                fg_color="#00C853", hover_color="#009624", # Bright Green
                corner_radius=13,
    ...
    def external_message(self, sender, content, msg_type, metadata):
        """Helper to receive messages from other threads (services.py)."""
        msg = self.storage.add_message(sender, content, msg_type, metadata)
        self._render_message(msg)
        self._update_count()
        self._scroll_to_bottom()
        
        # Tray notification for incoming from phone
        if sender == "phone" and hasattr(self.cp, '_tray_notify'):
             preview = content[:50] + ("..." if len(content) > 50 else "")
             self.cp._tray_notify("New Message", f"📱 {preview}")
...
# ═════════════════════════════════════════════════════════════════
#  EXTERNAL MODULE API (CALLED BY SERVICES.PY)
# ═════════════════════════════════════════════════════════════════

def append_chat_message(content, msg_type="text", is_user=False, metadata=None):
    """
    THREAD-SAFE Helper: Called by services.py or other background threads 
    to log a message and update the UI if it's open.
    """
    sender = "pc" if is_user else "phone"
    
    # Auto-generate metadata for files if missing (fixes missing open button)
    if msg_type == "file" and not metadata and os.path.exists(content):
        metadata = {
            "filename": os.path.basename(content),
            "filesize": str(os.path.getsize(content)),
            "filepath": content
        }

    # If UI is running, dispatch to it (thread-safe)
    if _active_tab:
        _active_tab.main_frame.after(0, 
            lambda: _active_tab.external_message(sender, content, msg_type, metadata)
        )
    else:
        # UI closed? Just write to JSON storage so it appears next time
        ChatStorage().add_message(sender, content, msg_type, metadata)
